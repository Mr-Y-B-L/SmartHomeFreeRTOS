...
#define PROID			"0aRguRYRAE"//产品ID

#define ACCESS_KEY		"N0tKZDJxb1lrd2NwM3hFY1pxMm1aRkhtTVdjb3FzbU0="
...
extern u8 temp,humi,fan_adj;
extern float mq2_vol,smog_th;
/*
************************************************************
*	函数名称：	OTA_UrlEncode
*
...
uint16_t OneNet_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"smog\":{\"value\":%.2f},", mq2_vol);
	strcat(buf, text);
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"fan_adj\":{\"value\":%d},", fan_adj);
	strcat(buf, text);

	memset(text, 0, sizeof(text));
	sprintf(text, "\"smog_th\":{\"value\":%.2f}", smog_th);
	strcat(buf, text);
...
void OneNet_RevPro(unsigned char *cmd)
{
	...
	cJSON *raw_json,*params_json,*led_json,*fan_json, *smog_th_json, *fan_adj_json;
...
//				char *data_ptr = NULL;
				...
				if(fan_json != NULL)
				{
					if(fan_json->type == cJSON_True) Fan_Set(FAN_ON);
					else Fan_Set(FAN_OFF);
				}
				
				smog_th_json = cJSON_GetObjectItem(params_json,"smog_th");
				if(smog_th_json != NULL)
				{
					smog_th = smog_th_json->valuedouble;
				}
				
				fan_adj_json = cJSON_GetObjectItem(params_json,"fan_adj");
				if(fan_adj_json != NULL)
				{
					fan_adj = fan_adj_json->valueint;
